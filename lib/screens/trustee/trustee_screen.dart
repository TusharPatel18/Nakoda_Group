...
import 'package:nakoda_group/models/trustee.dart';
...
class TrusteeListScreen extends StatefulWidget {
  @override
  _TrusteeListScreenState createState() => _TrusteeListScreenState();
}

class _TrusteeListScreenState extends State<TrusteeListScreen> {
  NetworkUtil _netUtil = new NetworkUtil();
  final GlobalKey<RefreshIndicatorState> _refreshIndicatorKey1 =
      new GlobalKey<RefreshIndicatorState>();
  Future<List<Trustee>> userdata;
  Future<List<Trustee>> userfilterData;
  ...
  Future<List<Trustee>> _refresh1() async {
    setState(() {
      userdata = _getUserData("user");
      userfilterData = userdata;
    });
  ...
  Future<List<Trustee>> _getUserData(String usertype) async {
    return _netUtil.post(RestDatasource.URL_TRUSTEE_LIST,
        body: {"usertype": usertype}).then((dynamic res) {
      final items = res.cast<Map<String, dynamic>>();
      List<Trustee> listofusers = items.map<Trustee>((json) {
        return Trustee.fromJson(json);
      }).toList();
      List<Trustee> revdata = listofusers.reversed.toList();
      return revdata;
    });
  }
  ...
  void updateSearchQuery(String newQuery) {
    setState(() {
      ...
      if (_searchQueryUser.toString().length > 0) {
        ...
        Future<List<Trustee>> items = userdata;
        List<Trustee> filter = new List<Trustee>();
        items.then((result) {
    ...
    if (isOffline) {
      ...
      return Scaffold(
        ...
        body: new RefreshIndicator(
          ...
          onRefresh: _refresh1,
          child: FutureBuilder<List<Trustee>>(
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              (data.Photo == null || data.Photo == "")
                                  ...
                                  : CircleAvatar(
                                      radius: 50,
                                      backgroundImage: NetworkImage(RestDatasource.Committee_IMAGE_URL + data.Photo),
                              ),
                              new SizedBox(
                                height: 15,
                              ),
